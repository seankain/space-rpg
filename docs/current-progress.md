...
- **Tests:** `Tests/SpaceRpg.Tests.csproj` — xunit over the engine-free sources (`Scripts/Data/`, `SaveRepository`, the dialogue graph/runtime/validation layer, the console command core), excluded from the Godot build; run with `dotnet test Tests`
...
First slice of the [NPC plan](plans/npc-system.md) (Phase 1 plus a taste of Phase 2's placement). Conversations are authored as serialized data and played by a homegrown dialogue box, editable in-game with the dialogue editor (see below); the [Yarn Spinner integration](plans/npc-dialogue-yarn.md) is still the eventual target for the text/authoring layer:

...
- **Save & assign** — `dialogue save [id]` validates then writes `res://Resources/Dialogue/<id>.dialogue.json` (or `<id>.yarn`, for a conversation authored in Yarn) and `Invalidate()`s the catalog so the edit plays live; `dialogue assign <npcId> <dialogueId>` points an NPC's role at a conversation and re-saves its `.tres`. Debug-gated, committed to the repo, the same workflow as NPC placement.

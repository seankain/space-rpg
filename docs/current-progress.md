...
- Four demo NPCs live in the Intro level:
  ...
  - **Vex** (`ChallengerRole`, patrols his stretch of deck) — challenge dialogue whose "Settle it" choice starts a real turn-based battle via `BattleManager.StartBattle`; winning records him in `GameState.DefeatedNpcs` (save version 6) and despawns him (`DespawnOnDefeat`), and defeated challengers skip spawning on reload.

...
| Role selection | A `PackedScene` per role on the definition (thin scene variants). Avoids fighting C#'s awkward runtime `SetScript` swap and keeps role scripts free to add scene structure later. |
